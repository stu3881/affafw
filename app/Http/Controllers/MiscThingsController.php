...
class MiscThingsController extends CRHBaseController
{
        ...
        //$this->WorkingArray                     = new  WorkingArray;
     ...
     */
     public function build_update_array_from_derived_field_names($requestFieldsArray) {
        $field_name_array = (array) json_decode($requestFieldsArray['encoded_field_name_array']);
        $field_name_keys = array_keys($field_name_array);
        $new_array = array();
        var_dump($field_name_array);
        $this->MyWorkingArray->show_related_arrays('00110',$this->MyWorkingArray->working_arrays,$requestFieldsArray['what_we_are_doing'],__LINE__);
        //var_dump($requestFieldsArray);$this->debugx('1100',__FILE__,__LINE__,__FUNCTION__);

        $update = 1; 
        $itsInTheArray = 1;
        $i = -1;
        $firstFieldName = $field_name_keys[0];
        //echo('$firstFieldName'.$firstFieldName);
        do {                       
          $i++;
          $derivedName = $firstFieldName . $i;
          if (key_exists($derivedName,$requestFieldsArray)){
            // if the first field has a value, save for all fields of that occurrrence
            if ($requestFieldsArray[$derivedName] <> $this->bypassed_field_name) {
              foreach ($field_name_array as $index=>$field_name){ 
                $derivedName = $index . $i;
                if(isset($requestFieldsArray[$derivedName])){
                $new_array[$field_name][] =
        ...
        foreach ($new_array as $index=>$value){
            $new_array[$index] = json_encode($value);
        }
        //echo $this->node_name;
        var_dump($requestFieldsArray);var_dump($new_array);
    ...
    public function update(REQUEST $request, $id)
    {
        ...
        //var_dump(array_keys($requestFieldsArray)); 
        //var_dump(($requestFieldsArray));
        //$this->debugx('1111',__FILE__,__LINE__,__FUNCTION__);
        if (array_key_exists('encoded_field_name_array',$requestFieldsArray)){
          $field_name_array = json_decode($requestFieldsArray['encoded_field_name_array']);
        ...
        switch ($what_we_are_doing) {
           ...
           case "maintain_modifiable_fields":
            ...
            case "ppv_define_business_rules":
                //var_dump(array_keys($requestFieldsArray));
                //var_dump(array_values($requestFieldsArray));
                $field_name_array = (array) json_decode($requestFieldsArray['encoded_field_name_array']);
                $field_name_keys = array_keys($field_name_array);
                //var_dump(array_keys($field_name_array));
                //var_dump(array_values($field_name_array));
               //$this->debugx('1111',__FILE__,__LINE__,__FUNCTION__);
                $just_the_ones_we_want = 
                $this->build_update_array_from_derived_field_names($requestFieldsArray);
            ...
            case "maintain_query_joins":
                ...
                $just_the_ones_we_want = $this->MyWorkingArray->recombine_joinee_field_types($working_arrays,$requestFieldsArray,$what_we_are_doing,$this->bypassed_field_name );
                $update = 1; 
       ...
       // ******
        if ($update){  
            ...
            switch ($request->what_we_are_doing) {
                ...
                case 'maintain_query_joins':
                 case 'ppv_define_query':
                   $requestFieldsArray = $just_the_ones_we_want;
                    //var_dump($requestFieldsArray);
                   $this->debugx('0111',__FILE__,__LINE__,__FUNCTION__);
                    break; 
                case "editUpdate":
            ...
            // ******
            $requestFieldsArray = $this->MyWorkingArray->flip_xref_prefixes($requestFieldsArray);
            //var_dump($requestFieldsArray);
            //$requestFieldsArray = $this->MyWorkingArray->flip_prefixes($working_arrays,$requestFieldsArray);
            //$this->debugx('1111',__FILE__,__LINE__,__FUNCTION__);

            $this->updateGetRedirect($this->key_field_name,$id,$requestFieldsArray,$request,$what_we_are_doing);

            //var_dump($coming_from);var_dump($what_we_are_doing);
            //$this->debugx('1111',__FILE__,__LINE__,__FUNCTION__);
            switch ($what_we_are_doing) {
    ...
    public function updateGetRedirect($key_field_name,$id,$requestFieldsArray,$request,$what_we_are_doing){
        $AllrequestFieldsArray=$request->all(); 
        $extra_requestFieldsArray = $requestFieldsArray;
         $extra_requestFieldsArray = $requestFieldsArray;
         //var_dump($extra_requestFieldsArray);
      
        //var_dump($AllrequestFieldsArray['to']);
        //var_dump($request->request->parameters);$this->snippet_model
        $this->debugx('0111',__FILE__,__LINE__,__FUNCTION__);
        $coming_from = $AllrequestFieldsArray['coming_from'];
        $what_we_are_doing = $AllrequestFieldsArray['what_we_are_doing'];
        //echo('coming_from ' .$coming_from);

        //$this->debugx('1110',__FILE__,__LINE__,__FUNCTION__);
        //echo('coming_from ' .$coming_from);
        switch ($coming_from) {
            case 'select_fields':
                $field_name = $AllrequestFieldsArray['field_name'];
                $to = json_encode($AllrequestFieldsArray['to']);
                $requestFieldsArray = array($field_name =>json_encode($AllrequestFieldsArray['to']));
                var_dump($requestFieldsArray);
                var_dump($to);
                //$this->debugx('1111',__FILE__,__LINE__,__FUNCTION__);
                $this->debugx('1111',__FILE__,__LINE__,__FUNCTION__);
                ...
                $requestFieldsArray = 
                    $this->MyWorkingArray->flip_xref_prefixes($requestFieldsArray);
                var_dump($requestFieldsArray);
                $this->debugx('1111',__FILE__,__LINE__,__FUNCTION__);
                ...
                break;
            case 'reportDefMenuEdit':
                //$update_array = array('report_name'=>$AllrequestFieldsArray['report_name']);
                $requestFieldsArray = array('report_name'=>$AllrequestFieldsArray['report_name']);
                break;
            case 'ppv_update':
                //var_dump($requestFieldsArray);
                //$this->debugx('1111',__FILE__,__LINE__,__FUNCTION__);
        ...
        switch ($what_we_are_doing) {
            case 'maintain_query_joins':
                //var_dump($extra_requestFieldsArray);
                $requestFieldsArray = $extra_requestFieldsArray;
                //*
                //$this->debugx('1111',__FILE__,__LINE__,__FUNCTION__);
            case 'select_fields':
            case 'reportDefMenuEdit':
...
// ******
               ...
               break;
            default :
                //$query_result = Maillist::where($key_field_name,  '=', $id)->update($requestFieldsArray);
                break;
            }
